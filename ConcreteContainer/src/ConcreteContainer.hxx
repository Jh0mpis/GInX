/**
 * \file ConcreteContainer.hxx
 * \brief ConcreteContainer class definition.
 *
 * The following file contains the definition of the ConcreteContainer class and
...
#ifndef CONCRETECONTAINER_HXX
#define CONCRETECONTAINER_HXX
...
// #############################################################################
//                   ConcreteContainer::CLASS INITIALIZATION
// #############################################################################
 ...
 * velocity of the Particles in the simulation.
 */
template <typename DerivedContainer, typename StructType>
class ConcreteContainer
    : public BaseContainer::BaseParticleContainer<DerivedContainer,
                                                  StructType> {
...
public:
   ...
   */
  using Base =
      BaseContainer::BaseParticleContainer<DerivedContainer, StructType>;
  using Base::Base;
  ...
  virtual void normalize_velocity(const amrex::MultiFab &metric,
                                  const int level) = 0;

  void redistribute_particles();
}; // ConcreteContainer class

// ##############################################################################
//                   ConcreteContainer::METHODS DECLARATION
// ##############################################################################
...
AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE CCTK_ATTRIBUTE_ALWAYS_INLINE
    amrex::GpuArray<CCTK_REAL, StructType::n_attributes + 3>
    ConcreteContainer<DerivedContainer, StructType>::compute_rhs(
        const amrex::GpuArray<CCTK_REAL, StructType::n_attributes + 3> &u,
        const CCTK_REAL &t, amrex::Array4<CCTK_REAL const> const &lapse,
 ...
 */
template <typename DerivedContainer, typename StructType>
void ConcreteContainer<DerivedContainer, StructType>::evolve(
    const amrex::MultiFab &lapse, const amrex::MultiFab &shift,
    const amrex::MultiFab &metric, const amrex::MultiFab &curv,
...
} // ConcreteContainer::evolve

template <typename DerivedContainer, typename StructType>
void ConcreteContainer<DerivedContainer, StructType>::redistribute_particles() {
  CCTK_INFO("Redistributing particles");
} // ConcreteContainer::redistribute_particles

} // namespace GInX

#endif // !CONCRETECONTAINER_HXX
